...
class MusicPlayTool(BaseTool):
    ...
    TOOL_DESCRIPTION = "Play music immediately — streams YouTube/SoundCloud via mpv+yt-dlp with no browser, or plays a local audio file. Audio starts in the background automatically."
    EXECUTION_TARGET = "client"
    PARAMS_SCHEMA: Dict[str, Any] = {
        "title": {"type": "string", "required": True, "description": "Song or track name"},
        "artist": {"type": "string", "required": False, "description": "Artist or band — improves search match"},
        "album": {"type": "string", "required": False, "description": "Album name — narrows ambiguous titles"},
        "source": {"type": "string", "required": False, "default": "youtube", "description": "'youtube' | 'soundcloud' | 'local'"},
        "file_path": {"type": "string", "required": False, "description": "Absolute path to local audio file — required only when source='local'"},
        "session_id": {"type": "string", "required": False, "default": "default", "description": "ID to track this session for stop/status control"},
    }
    OUTPUT_SCHEMA: Dict[str, Any] = {
        "success": {"type": "boolean"},
        "data": {"source": {"type": "string"}, "title": {"type": "string"}, "artist": {"type": "string"}, "album": {"type": "string"}, "query": {"type": "string"}, "file_path": {"type": "string"}, "file_name": {"type": "string"}, "format": {"type": "string"}, "session_id": {"type": "string"}, "process_id": {"type": "integer"}, "started_at": {"type": "string"}},
        "error": {"type": "string"},
    }
    EXAMPLES = [{"user_utterance": "play some music", "inputs": {"title": "Bohemian Rhapsody"}}]
    SEMANTIC_TAGS = ["media", "music", "play"]
    TOOL_CATEGORY = "media"
...
class MusicStopTool(BaseTool):
    ...
    # ── Plugin-shipped tool metadata ────────────────────────────────────
    TOOL_DESCRIPTION = "Stop one or all active music sessions launched by music_play."
    EXECUTION_TARGET = "client"
    PARAMS_SCHEMA: Dict[str, Any] = {
        "session_id": {"type": "string", "required": False, "default": "default", "description": "Session to stop"},
        "force": {"type": "boolean", "required": False, "default": False, "description": "Force-kill (SIGKILL) instead of graceful terminate (SIGTERM)"},
        "stop_all": {"type": "boolean", "required": False, "default": False, "description": "Stop every active session at once"},
    }
    OUTPUT_SCHEMA: Dict[str, Any] = {
        "success": {"type": "boolean"},
        "data": {"session_id": {"type": "string"}, "process_id": {"type": "integer"}, "method": {"type": "string"}, "was_playing": {"type": "object"}, "stopped_at": {"type": "string"}},
        "error": {"type": "string"},
    }
    EXAMPLES = [{"user_utterance": "stop the music"}]
    SEMANTIC_TAGS = ["media", "music", "stop"]
    TOOL_CATEGORY = "media"
...
class MusicStatusTool(BaseTool):
    """Two-layer audio status: our tracked sessions + OS-level audio scan."""

    # ── Plugin-shipped tool metadata ────────────────────────────────────
    TOOL_DESCRIPTION = "Two-layer audio status: (1) our tracked sessions with full metadata, (2) OS-level scan of every process emitting audio via pactl / CoreAudio / WASAPI."
    EXECUTION_TARGET = "client"
    PARAMS_SCHEMA: Dict[str, Any] = {
        "session_id": {"type": "string", "required": False, "description": "Filter our sessions to one ID. Omit to list all."},
        "include_system": {"type": "boolean", "required": False, "default": True, "description": "Run OS-level audio scan (pactl / CoreAudio / WASAPI)"},
    }
    OUTPUT_SCHEMA: Dict[str, Any] = {
        "success": {"type": "boolean"},
        "data": {"our_sessions": {"type": "object"}, "system_audio": {"type": "object"}, "timestamp": {"type": "string"}},
        "error": {"type": "string"},
    }
    EXAMPLES = [{"user_utterance": "what's playing right now"}]
    SEMANTIC_TAGS = ["media", "music", "status"]
    TOOL_CATEGORY = "media"
...
class MusicPauseTool(BaseTool):
    ...
    # ── Plugin-shipped tool metadata ────────────────────────────────────
    TOOL_DESCRIPTION = "Pause an active playback session by suspending the player process. Audio stops immediately; position is preserved. Resume with music_resume."
    EXECUTION_TARGET = "client"
    PARAMS_SCHEMA: Dict[str, Any] = {
        "session_id": {"type": "string", "required": False, "default": "default", "description": "Session to pause"},
    }
    OUTPUT_SCHEMA: Dict[str, Any] = {
        "success": {"type": "boolean"},
        "data": {"session_id": {"type": "string"}, "process_id": {"type": "integer"}, "method": {"type": "string"}, "title": {"type": "string"}, "artist": {"type": "string"}, "progress": {"type": "object"}, "paused_at": {"type": "string"}},
        "error": {"type": "string"},
    }
    EXAMPLES = [{"user_utterance": "pause the music"}]
    SEMANTIC_TAGS = ["media", "music", "pause"]
    TOOL_CATEGORY = "media"
...
class MusicResumeTool(BaseTool):
    ...
    # ── Plugin-shipped tool metadata ────────────────────────────────────
    TOOL_DESCRIPTION = "Resume a paused playback session. Accumulated pause time is tracked so progress remains accurate."
    EXECUTION_TARGET = "client"
    PARAMS_SCHEMA: Dict[str, Any] = {
        "session_id": {"type": "string", "required": False, "default": "default", "description": "Session to resume"},
    }
    OUTPUT_SCHEMA: Dict[str, Any] = {
        "success": {"type": "boolean"},
        "data": {"session_id": {"type": "string"}, "process_id": {"type": "integer"}, "method": {"type": "string"}, "title": {"type": "string"}, "artist": {"type": "string"}, "progress": {"type": "object"}, "resumed_at": {"type": "string"}},
        "error": {"type": "string"},
    }
    EXAMPLES = [{"user_utterance": "resume the music"}]
    SEMANTIC_TAGS = ["media", "music", "resume"]
    TOOL_CATEGORY = "media"
