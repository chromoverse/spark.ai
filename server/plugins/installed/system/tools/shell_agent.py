...
class ShellAgentTool(BaseTool):
    ...
    TOOL_DESCRIPTION = "Run guarded PowerShell or shell steps with LLM guidance until the task is finished. Set working_dir to the target directory from SYSTEM PATHS when the user references a known folder (desktop, downloads, documents, etc.)."
    EXECUTION_TARGET = "server"
    PARAMS_SCHEMA: Dict[str, Any] = {
        "goal": {"type": "string", "required": True},
        "working_dir": {"type": "string", "required": False, "description": "Starting directory — use SYSTEM PATHS when user references a known folder"},
        "max_steps": {"type": "integer", "required": False, "default": 6},
        "allow_network": {"type": "boolean", "required": False, "default": False},
    }
    OUTPUT_SCHEMA: Dict[str, Any] = {
        "success": {"type": "boolean"},
        "data": {"final_answer": {"type": "string"}, "steps": {"type": "array"}, "step_count": {"type": "integer"}, "working_dir": {"type": "string"}, "completed_at": {"type": "string"}},
        "error": {"type": "string"},
    }
