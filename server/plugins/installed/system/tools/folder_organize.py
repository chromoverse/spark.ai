...
class FolderOrganizeTool(BaseTool):
    """Organizes a folder intelligently using LLM categorization."""

    TOOL_DESCRIPTION = "Organize folder structure"
    EXECUTION_TARGET = "server"
    PARAMS_SCHEMA: Dict[str, Any] = {"path": {"type": "string", "required": True}}
    OUTPUT_SCHEMA: Dict[str, Any] = {"success": {"type": "boolean"}, "data": {"files_affected": {"type": "integer"}, "action_performed": {"type": "string"}, "organized_at": {"type": "string"}}, "error": {"type": "string"}}
    EXAMPLES = [{"user_utterance": "organize my downloads folder", "inputs": {"path": "~/Downloads"}}, {"user_utterance": "organize my desktop", "inputs": {"path": "~/Desktop"}}]
    SEMANTIC_TAGS = ["file_system", "folder", "organize"]
    ...
    async def _execute(self, inputs: Dict[str, Any]) -> ToolOutput:
        ...
        await tl.log_step("Analyzing file types with AI…")
...
{{
  ...
  "summary": "One sentence describing what was organized."
        ...
        breakdown = ", ".join(f"{cat}: {n}" for cat, n in counts.most_common())
        await tl.log_step(f"Categories: {breakdown}")
...
class FolderCleanupTool(FolderOrganizeTool):
    ...
    TOOL_DESCRIPTION = "Organize and clean folders"
    EXECUTION_TARGET = "server"
    PARAMS_SCHEMA: Dict[str, Any] = {"path": {"type": "string", "required": True}, "action": {"type": "string", "required": True}}
    OUTPUT_SCHEMA: Dict[str, Any] = {"success": {"type": "boolean"}, "data": {"files_affected": {"type": "integer"}, "action_performed": {"type": "string"}, "cleaned_at": {"type": "string"}}, "error": {"type": "string"}}
    EXAMPLES = [{"user_utterance": "clean up my desktop", "inputs": {"path": "~/Desktop"}}]
    SEMANTIC_TAGS = ["file_system", "folder", "cleanup"]
    TOOL_CATEGORY = "file_management"
    METADATA: Dict[str, Any] = {"summary_tts": True}
