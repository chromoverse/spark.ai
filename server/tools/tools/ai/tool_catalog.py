...
- Output: summary counts or a normalized detail/params payload for matching tools.
"""
...
class ToolCatalogTool(BaseTool):
    ...
    Params:
    - tool_name: optional exact tool name for detail/params lookup.
    - category: optional registry category filter for summary view.
    - execution_target: optional target filter, usually "server" or "client".
    - view: "summary", "detail", or "params".
    - include_examples: include registry examples when detail/params are returned.

    Output:
    - view: the normalized response view.
    - total_tools: number of matching tools.
    - categories: known registry categories for summary responses.
    - by_target: summary counts grouped by execution target.
    - tools: matching summary entries when view="summary".
    - tool: single normalized tool payload when view is "detail" or "params".
    """
