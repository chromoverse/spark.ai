# app/kernel/client_executor.py
"""
Client Tool Executor

Executes client-side tools (app_open, folder_create, etc.) using
the shared tool registry and loader. Uses kernel execution models.
...
class ClientToolExecutor:
    ...
    Uses kernel execution models (no client_core dependency)
    Just dict lookup — SUPER FAST
    """
    
    def __init__(self):
        self.schema_registry = get_schema_registry()
        logger.info("Client Tool Executor initialized (kernel)")
    ...
    async def execute(self, task: TaskRecord, resolved_inputs: dict) -> TaskOutput:
        ...
        Args:
            task: TaskRecord from kernel execution models
