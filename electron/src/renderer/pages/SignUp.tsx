...
import type { OnboardingDraft, OnboardingLocationState } from "./Onboarding/types";
import { toast } from "sonner";
import MinimalHeader from "@/components/local/MinimalHeader";

type SignUpLocationState = { onboardingDraft?: OnboardingDraft };

function SignUp() {
  const navigate = useNavigate();
  const location = useLocation();
  const dispatch = useAppDispatch();
  const onboardingDraft = ((location.state as SignUpLocationState | null) ?? null)?.onboardingDraft || null;
      ...
      if (data.access_token && data.refresh_token) {
        ...
        navigate("/auth/onboarding", {
          ...
          state: onboardingDraft
            ? ({ onboardingDraft, skipIntro: true, resumeAtReview: true } satisfies OnboardingLocationState)
            : undefined,
        });
      }
    } catch (error: any) {
